...
![Interfaz de la calculadora balística Iron Nest](screenshots/es-419.jpg)
...
1. **Configurar la posición del cañón**: haz clic en el botón «Gun Position» en la parte superior de la pantalla y elige el bloque (A–T), la fila (1–10) y las subcoordenadas (X/Y, 0–9) en la ventana emergente.
2. **Configurar las coordenadas del objetivo**: en el panel «Target Position», ingresa las coordenadas del objetivo de la misma manera.
3. **Elegir la carga**: selecciona un valor de carga de 1 a 6. Las opciones que superan el alcance actual (cada carga alcanza como máximo 5 km) se desactivan automáticamente, y la selección cambia a un valor válido si la distancia cambia y lo supera.
4. **(Opcional) Configurar la hora de impacto del objetivo**: abre la configuración (ícono de engranaje) y activa «Enable impact time / fire time», luego ingresa la hora de impacto del objetivo (formato de 24 horas) en «Target Impact Time» — el resultado calculará la hora de disparo correspondiente.
